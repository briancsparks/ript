...
func Cheat2(srcDir, destDir, tname string) error {
  ...
  if ConfigUseVersion() == 1 {
    return cheat3(srcDir, destDir, tname, nocopy, replacements)
  }
  ...
  err = fs.WalkDir(srcFS, root, func(shortPath string, d fs.DirEntry, err error) error {
    ...
    if ConfigUseVersion() == 2 {
      if d.IsDir() {
        if ConfigClobber() {
          err = os.MkdirAll(destPath, os.FileMode(0755)) // TODO: Magic number
        }
      } else {
        //if ConfigClobber() {
        ...
        putFileTotallyBroken(destPath, srcPath, srcFd, replacements, 0644) // TODO: Magic number
        //}
      }
...
func cheat3(srcDir, destDir, tname string, nocopy map[string]string, replacements map[string]string) error {
  ...
  for {
    ...
    if ConfigClobber() {
      ...
      func(dest string) {
        ...
        //destFd, err := os.Create(dest)
        destFd, err := os.OpenFile(dest, os.O_APPEND|os.O_CREATE|os.O_WRONLY, os.FileMode(hdr.Mode)) // TODO: Magic number
        if err != nil {
          log.Panic(err)
        }
        defer destFd.Close()
